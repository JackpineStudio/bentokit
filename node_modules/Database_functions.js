...
function getObjectsFromDatabase(callback, connection) {
	...
	query
	.on('result', function(row){
		var object = new AppObject(row['id'], row['name'], row['link'], row['image'], row['c1'], row['c2'], row['rating']);
		objects.push(object);
	})
	.on('end', function() {
		closeConnection(connection);
...
function saveApp(app, connection) {
	//deleteapp
	var id = app['id'];
	console.log("ID", id);
	
	//name, link, image, c1, c2, rating
	var object = new AppObject(id, app['name'], app['link'], app['image'], app['category1'], app['category2'], app['rating']);
	deleteAppFromID(id, "apps", function() {
		if(usePool) {
			pool.getConnection(function(err, poolConnection) {
				insertWithID(object, poolConnection);
			});
		} else {
			var connection = connectToDatabase();
			insertWithID(object, connection);
		}
	});
...
function insertWithID(object, connection) {
	if (object != null) {
		...
		.on('end', function() {
			if (count == 0) {
				var category1 = object.getC1(), category2 = object.getC2();
				var sqlQuery = "INSERT into apps (id, name, link, image, c1, c2, rating) values(?, ?, ?, ?, ?, ?, ?)";
				var query = connection.query(sqlQuery, [object.getID(), object.getName(), object.getLink(), object.getImage(), object.getC1(), object.getC2(), object.getRating()]);
				Logger.log(0, "Executing query '" + query.sql + "'");
				query.on('end' ,function() {
					console.log(0, "Succesfully inserted item into the database");
					closeConnection(connection);
...
exports.saveApp = function(app, callback) {
	if(usePool) {
		pool.getConnection(function(err, poolConnection) {
			saveApp(app, poolConnection, callback);
		});
	} else {
		var connection = connectToDatabase();
		saveApp(app, connection, callback);
	}
}
