...
function getApp(link, object, connection) {
	var sqlQuery = 'SELECT * FROM apps WHERE link = ?';	
	var query = connection.query(sqlQuery, link);
	Logger.log(0, "Executing query '" + query.sql + "'");
	object = new AppObject("", "", "", "", "", -1);
	query
	.on('result', function(row){
...
exports.getApp = function(link, object) {
	if(usePool) {
		pool.getConnection(function(err, poolConnection) {
			getApp(link, object, poolConnection);
		});
	} else {
		var connection = connectToDatabase();
		getApp(link, object, connection);
		console.log(object);
	}
